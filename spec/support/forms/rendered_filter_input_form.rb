...
RSpec.shared_context "with rendered filter input form" do
  ...
  def vc_render_filter_form(form_class = described_class, filter: self.filter,
                            additional_attributes: self.additional_attributes, active: self.active)
  ...
  shared_examples "hidden when inactive" do
    context "when inactive" do
      ...
      it "renders the row with hidden attribute" do
        expect(rendered_form).to have_element "data-filter-name": filter.name.to_s,
                                             hidden: "hidden",
                                             visible: :all
      end
    end
