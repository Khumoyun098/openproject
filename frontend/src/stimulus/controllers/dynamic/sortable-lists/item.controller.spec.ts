...
describe('Sortable lists item controller', () => {
  ...
  describe('Stimulus application wiring', () => {
    ...
    function renderBacklogsRows(itemIds:string[]) {
      fixture.innerHTML = `
      ...
      return Array.from(fixture.querySelectorAll<HTMLElement>('.Box-row'));
    }
    ...
    it('uses the handle target as the drag handle and keyboard handle', async () => {
      ...
      expect(article.getAttribute('role')).toEqual('button');
    ...
    it('describes the handle with hidden drag instructions', async () => {
      ...
      expect(description?.textContent).toContain('Press Space');
    });

    it('picks up the item for keyboard sorting with Space on the focused handle', async () => {
      const { row, article } = renderBacklogsRow();
      ...
      expect(row.dataset.dragging).toEqual('source');
      expect(article.getAttribute('aria-pressed')).toEqual('true');
    });

    it('moves the keyboard drop indicator down within the current list', async () => {
      const [firstRow, secondRow, thirdRow] = renderBacklogsRows(['1', '2', '3']);
      const handle = secondRow.querySelector<HTMLElement>('article')!;

      await nextFrame();
      handle.dispatchEvent(new KeyboardEvent('keydown', { bubbles: true, cancelable: true, key: ' ' }));
      handle.dispatchEvent(new KeyboardEvent('keydown', { bubbles: true, cancelable: true, key: 'ArrowDown' }));

      expect(firstRow.hasAttribute('data-drop-position')).toBe(false);
      expect(secondRow.hasAttribute('data-drop-position')).toBe(false);
      expect(thirdRow.dataset.dropPosition).toEqual('bottom');
    });

    it('emits the keyboard drop request with the selected previous item', async () => {
      const [, secondRow, thirdRow] = renderBacklogsRows(['1', '2', '3']);
      const list = fixture.querySelector<HTMLElement>('[data-sortable-lists-target="list"]')!;
      const handle = secondRow.querySelector<HTMLElement>('article')!;
      const dropEvents:CustomEvent[] = [];

      list.addEventListener('sortable-lists:item-keyboard-drop', (event) => {
        dropEvents.push(event as CustomEvent);
      });
      ...
      expect(dropEvents[0].detail).toEqual(expect.objectContaining({
        list,
        previousItemId: '3',
        sourceElement: secondRow,
      }));
      expect(dropEvents[0].detail.sourceData).toEqual(expect.objectContaining({
        itemId: '2',
        moveUrl: '/move',
        type: 'work_package',
      }));
      expect(secondRow.hasAttribute('data-dragging')).toBe(false);
      expect(thirdRow.hasAttribute('data-drop-position')).toBe(false);
      expect(handle.getAttribute('aria-pressed')).toEqual('false');
    });

    it('cancels keyboard sorting with Escape', async () => {
      const [, secondRow, thirdRow] = renderBacklogsRows(['1', '2', '3']);
      const handle = secondRow.querySelector<HTMLElement>('article')!;
      ...
      handle.dispatchEvent(new KeyboardEvent('keydown', { bubbles: true, cancelable: true, key: ' ' }));
      handle.dispatchEvent(new KeyboardEvent('keydown', { bubbles: true, cancelable: true, key: 'ArrowDown' }));
      handle.dispatchEvent(new KeyboardEvent('keydown', { bubbles: true, cancelable: true, key: 'Escape' }));
    ...
    it('does not start keyboard sorting from interactive elements inside the handle', async () => {
      const { row, article } = renderBacklogsRow();
      ...
      expect(article.getAttribute('aria-pressed')).toEqual('false');
    });

    it('does not start keyboard sorting while the sortable lists root is moving another item', async () => {
      fixture.innerHTML = `
        <div data-controller="sortable-lists" data-sortable-lists-moving="true">
          <ul data-sortable-lists-target="list" data-sortable-lists-list-type="backlog" data-sortable-lists-list-id="">
            <li
              class="Box-row"
              data-controller="sortable-lists--item"
      ...
      const row = fixture.querySelector<HTMLElement>('.Box-row')!;
      const handle = row.querySelector<HTMLElement>('article')!;
      const event = new KeyboardEvent('keydown', {
        bubbles: true,
        cancelable: true,
        key: ' ',
