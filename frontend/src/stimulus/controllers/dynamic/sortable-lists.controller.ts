...
export default class SortableListsController extends Controller<HTMLElement> {
  ...
  private readonly handleKeyboardDropBound = this.handleKeyboardDrop.bind(this);
  ...
  connect():void {
    ...
    this.element.addEventListener(sortableItemKeyboardDropEvent, this.handleKeyboardDropBound);
  }

  disconnect():void {
    this.monitorCleanupFn?.();
    this.monitorCleanupFn = undefined;
    this.element.removeEventListener(sortableItemKeyboardDropEvent, this.handleKeyboardDropBound);
  ...
  private async handleDrop({ location, source }:ElementDropPayload) {
    ...
    const targetItem = location.current.dropTargets.find(({ data, element }) => (
      isSortableItemData(data) && element instanceof HTMLElement
    ));
  ...
  private handleKeyboardDrop(event:CustomEvent<SortableItemKeyboardDropDetail>):void {
    const {
      list,
      previousItemId,
      sourceData,
    ...
    if (this.moving || !isSortableItemData(sourceData) || !this.element.contains(list)) {
      return;
    }

    if (!acceptsSortableItemType({
      acceptedType: this.acceptedType,
