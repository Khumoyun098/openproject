...
RSpec.describe Backlogs::WorkPackageCardListItemComponent, type: :component do
  ...
  describe "#row_args" do
    ...
    context "when the user cannot manage sprint items" do
      ...
      it "does not mark the row as draggable" do
        ...
        expect(item.row_args[:tabindex]).to eq(0)
      end
    end
  end
  ...
  describe "URL derivation by container" do
    context "with a sprint container" do
      it "uses the shared move route" do
        ...
        card = page.find(".op-backlogs-story")
  ...
  describe "#card" do
    ...
    it "does not wire the card as the draggable item" do
      ...
      expect(rendered_card).to have_no_css(".op-backlogs-story[data-controller~='sortable-lists--item']")
      expect(rendered_card).to have_no_css(".op-backlogs-story[data-sortable-lists--item-move-url-value]")
      expect(rendered_card).to have_no_css(".op-backlogs-story[draggable='true']")
    end
